      ...
      <div>
        <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
            ...
            <div onClick={() => (expand ? setExpand(false) : setExpand(true))}
             className='group relative flex items-center justify-center hover:bg-grey-500/20 transition-all
        ...
        {!expand && (
          <div className='absolute w-max left-1/2 -translate-x-1/2 -top-12 opacity-0 group-hover:opacity-100 
      ...
      <div className={`mt-8 text-white/25 text-sm ${expand ? "block" : "hidden"}`}>
        ...
        <div>
          ...
          <div onClick={user ? null : openSignIn}
           className={`flex items-center ${expand ? "hover:bg-white/10 rounded-lg" : "justify-center w-full"} gap-3 text-white/60 text-sm 
          p-2 mt-2 cursor-pointer`}>
            {user ? <UserButton/>  : 
            <Image src={assets.profile_icon} alt=''  className='w-7' />
            }
            {expand && <span className='text-white text-sm'>My Profile</span>}
          </div>
